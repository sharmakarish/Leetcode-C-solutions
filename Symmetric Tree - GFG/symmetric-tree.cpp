...
class Solution{
    ...
    // return true/false denoting whether the tree is Symmetric or not
      bool symmetric(struct Node *lst,struct Node *rst){
        if(lst==NULL && rst==NULL)return true;
        if(lst==NULL || rst==NULL)return false;  //koi bhi ek null to false
        if(lst->data!=rst->data)return false;  //data not equal to false
